...
const getFormatDate = (...options) => {
    ...
    const parseOptions = Validate_1.default.optionsFormatDate(options);
    const timestamp = Validate_1.default.timestamp(parseOptions?.timestamp);
    const local = Validate_1.default.locale(parseOptions?.local ?? "pt-PT");
    const timezone = Validate_1.default.timezone(parseOptions?.timezone);
    const hour12 = Validate_1.default.hour12(parseOptions?.hour12);
    ...
    for (const key of [{ hourLong: { format: "{hh}", hour: "2-digit" } }, { hourShort: { format: "{h}", hour: "2-digit" } }]) {
        for (const data of Object.values(key)) {
            ...
            content = content.replace(key[keyObject]?.format ?? "", String(hour).padStart((key[keyObject]?.format?.length ?? 0) - 2, "0"));
        }
    }
